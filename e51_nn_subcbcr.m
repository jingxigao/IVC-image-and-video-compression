...
for f = 21:40
    ...
    %down_sample Cb and Cr 
    err_Cb_sub=err_Cb(1:2:ss1,1:2:ss2);
    err_Cr_sub=err_Cr(1:2:ss1,1:2:ss2);
...
for i = 1:8:size(err_Y,1)
    for j = 1:8:size(err_Y,2)
        ...
        [block_Cb] = DCT8x8(err_Cb_sub(i:i+7, j:j+7));
        [block_Cr] = DCT8x8(err_Cr_sub(i:i+7, j:j+7));
