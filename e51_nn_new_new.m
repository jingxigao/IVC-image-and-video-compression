...
%                 end   
%             end
              RECONS_Y_2=imresize(RECONS_image_Y(:,:,f-20),2,'bilinear');
              RECONS_Cb_2=imresize(RECONS_image_Cb(:,:,f-20),2,'bilinear');
              RECONS_Cr_2=imresize(RECONS_image_Cr(:,:,f-20),2,'bilinear');
for i = 1:8:ss1
    for j = 1:8:ss2        
        vector2=ORIGINAL_image_Y(i:i+7,j:j+7);
        sk=sum(sum(vector2));
%       sk_1=sum(sum(recon_Y(i:i+7,j:j+7,f-20)));
%       delta=abs(sk-sk_1);
        SAD_min=inf;        
       % mv_block=[0,0];
  for m=-4:+4
      for n=-4:+4
          if (2*i+m>0 && 2*i+m+7<=2*ss1 && 2*j+n>0 && 2*j+n+7<=2*ss2 )
          
              vector1=RECONS_Y_2(2*i+m:2*i+m+7,2*j+n:2*j+n+7);
              sk_1=sum(sum(vector1));
              delta=abs(sk-sk_1);
          if(delta<=SAD_min)
              SAD=sum(sum(abs(vector2-vector1)));
              if (SAD<SAD_min)
                  SAD_min=SAD;
                  mv_block=[m,n];
              end
            ...
            block_curr_Y = RECONS_Y_2(2*i+y:2*i+7+y, 2*j+x:2*j+7+x);
            block_curr_Cb = RECONS_Cb_2(2*i+y:2*i+7+y, 2*j+x:2*j+7+x);
            block_curr_Cr = RECONS_Cr_2(2*i+y:2*i+7+y, 2*j+x:2*j+7+x);
...
%% Reconstruct    
    ...
    for i = 1:8:ss1
        for j = 1:8:ss2
            ...
            r1 = M1(count,1);
            c1 = M1(count,2);
            block_curr_Y1 = RECONS_Y_2(2*i+r1:2*i+7+r1, 2*j+c1:2*j+7+c1);
            block_curr_Cb1 = RECONS_Cb_2(2*i+r1:2*i+7+r1, 2*j+c1:2*j+7+c1);
            block_curr_Cr1 = RECONS_Cr_2(2*i+r1:2*i+7+r1, 2*j+c1:2*j+7+c1);
